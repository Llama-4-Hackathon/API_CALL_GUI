...
def normalize_type(type_str):
    ...
    return mapping.get(type_str.lower(), type_str.lower())
...
# ------------- GRADIO HOOK FUNCTION ------------
# Keep global state across messages
def process_user_message(user_input, conversation, answers):
    ...
    if matched_key:
        ...
        # ✅ Optional: Auto-save if all required are filled
        required_fields = [k for k, v in answers.items() if v.get("Required")] 
        still_unanswered = [k for k in required_fields if answers[k].get("Value", "").strip() == ""]
        if not still_unanswered:
            save_json(ANSWER_FILE, answers)
            return "✅ All required fields are answered and saved. Thank you!", conversation, answers

    # Normal assistant follow-up
    response = chat_completion(conversation, answers)
    assistant_message = parse_response(response).strip()
    conversation.append({"role": "assistant", "content": assistant_message})
